...
int Debayer::DebayerContainer::ComputeChannels() {
    ...
    for (unsigned int readptr = 0; readptr < width * height * 3 / 2; readptr += 3) {
        ...
        if (row % 2 == 0) {
            ...
            imagegreen1gr[row * (width / 2) + col] = (((uint8_t) inputbuffer[readptr + 1] & 0x0F) << 4 |
                                                      (uint8_t) inputbuffer[readptr + 2] >> 4);
        } else {
            row = row / 2;
            col = col / 2;
            imagegreen2gr[row * (width / 2) + col] = inputbuffer[readptr];
            imagebluegr[row * (width / 2) + col] = (((uint8_t) inputbuffer[readptr + 1] & 0x0F) << 4 |
                                                    (uint8_t) inputbuffer[readptr + 2] >> 4);
        }
    }
}
